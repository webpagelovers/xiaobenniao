...
<template>
	<div class="page-product">
		<page-head :title="isEdit?'编辑产品':'新增产品'"></page-head>
		<nt-row class="operationBox " style="margin: 0;">
			<nt-col :span="12" class="f16">
				<i @click="close($event)" class="nt-icon nt-icon-arrow-left" style="cursor: pointer;"></i>&nbsp;&nbsp;{{isEdit?'编辑产品':'新增产品'}}
			</nt-col>
		...
		<nt-form ref="form" :model="form" label-width="150px" :rules="rules" class="contentBox">
			<!-- 基本信息 -->
			<div class="border_layout competing_infor">
				...
				<div class="competing_infor_main break">
					...
					<nt-form-item label="规格型号：" prop="specsModel">
						...
						<span class="mL5 col_cccccc">多种规格型号用逗号分隔，比如：蓝色，180，XL</span>
					</nt-form-item>
			...
			<div class=" competing_infor mT25">
				...
				<div class="competing_infor_xbox">
					<div class="col_cccccc">照片仅支持jpg、gif、png三种格式,大小不超过4MB</div>
					<nt-form-item label="产品外观及配件图：" class="is-required mB0" prop="surfacePhotos">
						<div class="col_cccccc">（最多上传5张）不带外包装的产品图，如有配件，需提供产品与配件放在一起拍摄的照片或产品与配件单独拍摄的照片</div>
						<div>
					...
					<nt-form-item label="产品外包装图：" class=" is-required mB0" prop="packingPhotos">
						<div class="col_cccccc">（最多上传5张）带着外包装的产品整体照片，可上传多个角度的照片</div>
						...
						<!--<div class="col_cccccc block">带着外包装的产品整体照片，可上传多个角度的照片</div>-->
					</nt-form-item>
					<nt-form-item label="品牌/型号/特殊标识图：" class=" is-required " prop="brandModelPhotos">
						<div class="col_cccccc">（最多上传5张）产品外包装或产品主体上标有品牌、型号位置的特写照片</div>
						...
						<!--<div class="col_cccccc block">产品外包装或产品主体上标有品牌、型号位置的特写照片</div>-->
					</nt-form-item>
				</div>
			</div>
			<!-- 包装信息 -->
			<div class=" competing_infor mT25">
				...
				<div class="competing_infor_xbox">
					<nt-form-item label="产品毛重：" prop="grossWeight">
						...
						<span class="mL5 col_cccccc">含外包装的重量，单位：KG</span>
					</nt-form-item>
					...
					<nt-form-item label="产品尺寸：" class="is-required">
						<nt-form-item class="inline_block" prop="">
							<nt-input v-model="form.length" class="w80" placeholder="长"></nt-input>
							<span class="mL5 mR5 col_999">X</span>
						</nt-form-item>
						...
						<span class="mL5 col_cccccc">外包装尺寸，长 X 宽 X 高，单位：CM</span>
					</nt-form-item>
				...
				<div class="competing_infor_main break">
					<nt-form-item label="HS编码：" prop="hsCode">
						...
						<span class="mL5 col_cccccc">
							<a href="/static/html/helpBianma.html" target="_blank" class="main_color">HS编码查询方法</a>
						</span>
					</nt-form-item>
					...
					<nt-form-item :label="'申报要素项 ' + (index + 1) + '：'" v-for="(item,index) in form.applyElements" :key="index" :prop="'applyElements.'+index+'.applyElementValue'" :rules="[{ required: true, message: '此项不能为空', trigger: 'blur' }]">
						...
						<nt-button v-if="form.applyElements.length > 1">删除</nt-button>
					</nt-form-item>
					<nt-form-item label="">
				...
				<div class="competing_infor_main break">
					<nt-table ref="multipleTable" :data="form.clearCustoms" tooltip-effect="dark" class="dialog_table">
						...
						<nt-table-column label="清关海关编码">
							<template slot-scope="scope">
								<nt-form-item label="" class="nt-form-item-no-label" :prop="'clearCustoms.'+scope.$index+'.customHsCode'" :rules="[{ required: true, message: '此项不能为空', trigger: 'blur' },
						...
						<nt-table-column label="关税税率">
							<template slot-scope="scope">
								<nt-form-item label="" class="nt-form-item-no-label" :prop="'clearCustoms.'+scope.$index+'.customTaxRate'" :rules="[{ required: true, message: '此项不能为空', trigger: 'blur' },
											 { type: 'string',pattern: new RegExp('^[0-9]{1,3}[\.]{1}[0-9]{1,2}$|^[0-9]{1,3}$') ,message: '整数位最多3位，小数2位',trigger: 'blur'}]">
						...
						<nt-table-column label="VAT/GST税率">
							<template slot-scope="scope">
								<nt-form-item label="" class="nt-form-item-no-label" :prop="'clearCustoms.'+scope.$index+'.vatGstTaxRate'" :rules="[{ required: true, message: '此项不能为空', trigger: 'blur' },
											 { type: 'string',pattern: new RegExp('^[0-9]{1,3}[\.]{1}[0-9]{1,2}$|^[0-9]{1,3}$') ,message: '整数位最多3位，小数2位',trigger: 'blur'}]">
			...
			<div class=" competing_infor mT25">
				...
				<div class="competing_infor_main rel">
					<nt-form-item label="联系人姓名：" prop="applyName">
						...
						<span class="mL5 col_cccccc"> 最多60个字符</span>
					</nt-form-item>
					<nt-form-item label="联系人电话：" prop="applyName">
						<nt-input v-model="form.contactTel" class="w240" :maxlength="20"></nt-input>
					</nt-form-item>
					<nt-form-item label="联系人邮箱：" prop="applyName">
...
export default {
		...
		return {
			...
			rules: {
				...
				applyNameCn: [
					...
					{ min: 0, max: 80, message: '最多 80 个字符', trigger: 'blur' },
					{ type: "string", pattern: /^[a-zA-Z ]+$/, message: '只能包含字母、下划线和空格', trigger: 'blur,change' },
				...
				volume: [
					{ required: true, message: '不能为空', trigger: 'blur' },
					{ type: 'string', pattern: new RegExp('^[0-9]{1,3}[\.]{1}[0-9]{1,6}$|^[0-9]{1,3}$'), message: '整数位最多3位，小数6位', trigger: 'blur,change' }
				],
				//监管证件/认证文件信息
				isUploadFile:[
	...
	methods: {
		...
		async getDetail(id) {
			...
			if(this.form.isLegalCheck == null){
				this.form.isLegalCheck = '0';
			}
		},
		//添加申报要素
		addApplyElement() {
