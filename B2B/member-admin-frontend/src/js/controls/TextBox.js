    ...
    X.prototype.controls.widget("TextBox",function (controlType) {
        ...
        function TextBox(elem,options) {
            ...
            if(this.option){
                ...
                this.option.onlyNumber ? this.onlyNumber(elem) : '';

                this.option["maxlength"] ? this.elem.attr("maxlength",this.option["maxlength"]) : '';

                this.option["minLength"] ? this.elem.attr("minLength",this.option["minLength"]) : '';

                this.option["disabled"] ? this.elem.attr("disabled",this.option["disabled"]) : '';
        ...
        TextBox.getTemplate = function (item) {
            ...
            input.type = 'text';
            input.className = 'default_input '+ item.className +' fL js-'+ item.name;
            item.maxLength? input.maxLength = item.maxLength: 1;
            item.minLength? input.minLength = item.minLength: 1;
        ...
        TextBox.positiveNumber = function (ele) {
            ele.onkeyup = function() {
                ...
                if( /\.\d\d$/.test(this.value) || isNaN(this.value) || this.value <= 0) {
        ...
        /*
         @method onlyNumber 设置文本框，只能输入数字
         */
        TextBox.prototype.onlyNumber = function (ele) {
            $(ele).on('keyup focus',function(event){
                if( !(/^\d+$/.test($(this).val())) || isNaN($(this).val()) || $(this).val() <= 0) {
                    event.returnValue=false;
                    $(this).val('');
                }
            });
