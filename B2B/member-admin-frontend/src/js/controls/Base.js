...
(function ($,X) {
    ...
    var ViewModel = function (elem,options) {
    	...
    	vm.prototype.lazyControls = function () {
    		return 'Upload Uploader UploaderNew WebUpload IntlTelInput'.split(' ');
    	};
		 ...
		 * @param  {[type]} options {
		 *         				lazy: true //是否需要懒加载大型control
		 * 					} 
		 * @return {[type]}      [description]
		 */
		vm.prototype.initControl = function (options) {
			var meta = this.getOption("meta") || {};
			var data = this.getOption("data") || {};
			var lazy = this.lazyControls();
			...
			if(this.elem){
				...
				list.each(function(index,element){
					...
					if(!_controls[pName] && (!options.lazy || (hasData || lazy.indexOf(ctrlType) === -1))){
						_controls[pName] = CS.getControl(ctrlType,item,meta[pName]||{});
						if(data && data[pName] != undefined){
							_controls[pName].setValue(data[pName]);
						}
					}
					else{
						_controls[pName] && (console.log("已经存在的" + pName));
					}
				});
			}
		};
		...
		//生成大型的control
		vm.prototype.initLazyControl = function () {
			var meta = this.getOption("meta") || {};
			var data = this.getOption("data") || {};
			var lazy = this.lazyControls();
			if(this.elem){
				...
				list.each(function(index,element){
					...
					ctrlType = item.attr(CONTROL_TYPE_TAG);
					pName = item.attr(PROPERTY_NAME_TAG);
					if(!_controls[pName] && lazy.indexOf(ctrlType) > -1){
						_controls[pName] = CS.getControl(ctrlType,item,meta[pName]||{});
						if(data && data[pName] != undefined){
							_controls[pName].setValue(data[pName]);
						}
