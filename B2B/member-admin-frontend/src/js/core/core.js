...
(function (X){ 
    ...
    var view = proto.object.create({
        ...
        //往页面上渲染数据，风格还是和loadTemplate一致
        renderData: function(data) {
            this.el.renderData(data)
        },
...
(function (win, X) {
	...
	var Router = function (uri) {
		...
		this.run = function (url,title,state,callback) {
...
(function (X){ 
    ...
    //用于懒加载依赖项，因为后台太慢，换种方式写了
    //由于有可能用到多个依赖项，所以暂时使用同步加载，即一个接着一个
    //当有依赖项需要提前加载时，就让他插队
    var lazyloadModules = proto.object.create({
        modeStatus: ['loading', 'loaded'],
        loadingModeStatus: 1,//加载的状态[loading, loaded]
        loadingModelName:  '',//当前加载的模块
        loadModelNames: [],
        oriModelNames: [],
        models: {},
        context: {
        ...
        init: function(modules) {
            var me = this
            this.loadModelNames = modules || []
            this.loadModelNames.forEach(function(item) {
                me.oriModelNames.push(item)
            })
            this._nextTick()
        },
        _exec: function(path) {
            var me  = this,
                key = me._removePrefix(path)

            me.addHooks(key, 0)
            X.prototype.require(path, function(model) {
                me.models[key] = model
                me.addHooks(key, 1)
                me._nextTick()
            })
        },
        _nextTick: function(path) {
            var me    = this,
                path  = path || me.loadModelNames[0],
                index = me.loadModelNames.indexOf(path)

            index > -1 && me.loadModelNames.splice(index, 1)
            path && me._exec(path)
        },
        ...
        /**
         * [getModel description]分三种情况：已加载、加载中、未加载
         * @param  {[type]}   name     [description]
         * @param  {Function} callback [description]
         * @return {[type]}            [description]
         */
        getModel: function(path, callback) {
            var me   = this, 
                mm   = null
                name = me._removePrefix(path)

            if (path.split('.').length === 1 && !me.models[name]) {
                me.oriModelNames.forEach(function(item) {
                    me._removePrefix(item) === path && (path = item)
                    return
                })
            }

            //已加载
            if (mm = me.models[name]) {
                callback(mm)
            } else {
                //加载中、未加载
                me.on('loaded', name, callback)
                !(me.loadingModeStatus === 0 && me.loadingModelName === name) && me._nextTick(path)
            }
        },
        addHooks: function(name, status) {
            var me = this
            me.loadingModeStatus = status
            me.loadingModelName  = name

            me.trigger(name, me.modeStatus[status])
        },
        trigger: function(name, status) {
            var me     = this,
                events = this.context.events
            events.forEach(function(item, index) {
                if (item.type === status && item.name=== name) {
                    events.splice(index, 1)
                    item.callback && item.callback(me.models[name])
                    return
                }
            })
        },
        on: function(type, name, callback) {
            var eve = {
                type: type,
                name: name,
                callback: callback
            }
            this.context.events.push(eve)
        }
    });
    proto.lazyloadModules = lazyloadModules;
