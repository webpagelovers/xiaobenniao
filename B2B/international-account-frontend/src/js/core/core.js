                    ...
                    ajaxOpt['success'] = function (data) {
                        ...
                        if (ajaxOpt.dataType === "script") {
                            ...
                            if (data.statusCode && (data.statusCode == '1000100' || data.statusCode == '-2') && new Xbn().parseURL(location.href).params.a != 'login') {
                                location.href = X.prototype.config.PATH_FILE.path.rootReceptionUrl+'/user/login.html?refer='+escape(location.href);
                                return false;
                            }
                            opt.callback(data);
...
(function (win,X){ 
    ...
    */
	...
	prototype.getRequest = (function (args) {
		...
		var getRequestHash = function (url) {
		...
		var letIE10 = $.browser["msie"]==true && parseInt($.browser["version"])<10;
    
    	return letIE10 ? getRequestHash : getRequest        
	}());
...
(function (win, X) {
    ...
    var RouterHash = (function (win) {
        ...
        hashRouter.run = function (url,title,state) {
            var mold = pro.getRequest();
            var mnew = pro.getRequest(url);
            if(JSON.stringify(mold)!=JSON.stringify(mnew)){
                this.setHistory(url,title,state);
                //runCallback 此处不需要调用，原因在于onhashchange会被浏览器监测并调用
                //this.runCallback(get_hash());
            }
        };

        hashRouter.back = function(){
            win.history.back();
        };
    ...
    RouterHash.prototype = pro.createObject(pro);
