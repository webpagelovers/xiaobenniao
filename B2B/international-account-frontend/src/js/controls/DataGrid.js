(function ($,X) {     
    ...
    */
    var DataGrid = function (element, config) {
        ...
        this._tbody = element.firstChild.tBodies[0];
    ...
    DataGrid.prototype = $.extend(DataGrid.prototype,{        
        ...
        */
        insertRow: function (data) {
            ...
            //this._tbody.appendChild(row.dom);
        ...
        */
        removeRow: function (row) {
            ...
            this._tbody.removeChild(row.dom);
            row.destroy();
        ...
        */
        ...
        @return 表格行信息
        */
        collectData : function (dirty) {
            var result = [],temp={};
            for (var i = 0; i < this.rows.length; i++) {
                temp = this.rows[i].getValue(dirty);
    ...
    DataRow.getEmptyRow = function (colspan,innerHTML,emptyRowRender) {
        ...
        if(emptyRowRender){
            ...
            cell.innerHTML = [
                ...
                '   <i></i>',
                '   <span>You have no sourcing request.</span>',
                '   <a>Request for quotation right now !</a>',
                '</div>'
            ].join('')
        }
        return row;
    ...
    */
    DataRow.prototype = $.extend(DataRow.prototype,{
        ...
        renderEdit: function (cell, data, field, column, index) {
            ...
            if(ctrlType){
                ...
                if(ctrlClazz){
                    ...
                    ctrl = X.prototype.controls.getControl(ctrlType,$(ctrlHtml));
        ...
        */
        ...
        @param data {Array} 行数据
        */
        getValue : function (dirty) {
           var result = {},temp,odata;
            for (var i = 0; i < this.grid.columns.length; i++) {
                var column = this.grid.columns[i];
                if(column["field"] && !(column["nodata"]==true)){
                    ...
                    if(column.itemCollector){
                        ...
                        if(ctrl){
                            ...
                            if(dirty){
                                if(column.formater){
                                    odata = column.formater.format(this.data,fieldName);
                                }else{
                                    odata = this.data[fieldName];
                                }
                                if(temp!=odata){
                                    result[fieldName] = temp;
                                }
                            }
                            else{
                                result[fieldName] = temp;
    ...
    */
    var SDataGrid = function (element,config) {
        ...
        this.header = element.firstChild.tHead;
        this._tbody = element.firstChild;
    ...
    $.extend(TDataRow.prototype,{
        ...
        */
        renderRow: function (data) {
            ...
            // var tmpl = $.tmpl(this.grid.$template.html(),data);
