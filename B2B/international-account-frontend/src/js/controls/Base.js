(function ($,X) {
    ...
    X.prototype.controls = X.prototype.controls || (function (X) {
        ...
        */
        var getControl = function(ctrlType,elem,option){
			...
			X.prototype.echo("没有相应的控件类型"+ ctrlType);
		...
		var getTemplate = function (item) {
			...
			X.prototype.echo("没有相应的控件类型，不能获得控件模板");
    ...
    var ViewModel = function (elem,options) {
		...
		vm.prototype.collectData = function (dirty ,odata) {
			var result = {};
			var value;
			var data = this.getOption("data") || odata || {};
			if(_controls){
				for(var k in _controls){
					value = _controls[k].getValue();
					if(dirty){
						if(data[k]!=value){
							result[k] = value;
						}
