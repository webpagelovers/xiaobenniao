                ...
                if(that.elem.find(".wrapUpload").length < that.maxNum){
                    if (response.data) {
                        ...
                        if (X.prototype.isFunction(that.options["uploadSuccessBackFill"])) {
                            that.options["uploadSuccessBackFill"](response,that.wrapData);
                        }else{
                            var a = '<div class="wrapUpload"><a href="' + uploadUrl + '?fileType=' + that.options.downloadType + '&filePath=' + response.data.path + '&fileName=' + response.data.fileName + '" target="_blank" class="accessory blueFont">' + response.data.fileName + '</a><span class="redFont cancel">X</span></div>';
