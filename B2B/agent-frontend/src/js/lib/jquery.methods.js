;(function (_$) {
    ...
    var imageVerificationController = X.config.imageVerification.imageVerificationController;
    var ruls = {
    ...
    _$.validator.addMethod("telCode", function (value, element) {
        ...
        _$.ajax({
            type: "PUT",
            url: imageVerificationController + "/VerificationCode/check?"+"mobile="+$("#phone").val()+"&verificationCode="+value,
    ...
    _$.validator.addMethod("verCode", function (value, element) {
        var url = $(element).parent().find("#code").attr("src").split("/");
        var flag = 1;
        _$.ajax({
            type: "POST",
            url: imageVerificationController + "/ImageVerificationCode/check/"+value+"/"+url[url.length-1],
            async: false, //同步方法，如果用异步的话，flag永远为1
            success: function (data) {
                if (data.statusCode == '2000000') {
                    flag = 0;
    ...
    _$.validator.addMethod("verifyMobile", function (value, element) {
        ...
        _$.ajax({
            type: "get",
            url: ROOT_PATH + "/member/verifyMobile/1/"+ value,//1为userType
            async: false, //同步方法，如果用异步的话，flag永远为1
            success: function (data) {
                if (data.statusCode == '2000000') {
                   flag = data.data[0].code;
