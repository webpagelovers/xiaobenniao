X.define("modules.bid.myBid",["model.bidModel", "data.addressData","modules.common.routerHelper"],function (bidModel, addressData,routerHelper) {

    //初始化视图对象
    var view = X.view.newOne({
        el: $(".xbn-content"),
        url: X.config.bid.tpl.myBid
    });
        ...
        "tabAll" :[
            {
                ...
                itemRenderer:{
                    render: function(data, field, index, grid){
                        if(data.noticeable == 1 && data["applyDate"]){
                            return "<span class='orange-font'>已变更</span>";
                        }else{
                            return "未变更";
                        }
                    }
                        ...
                        if(data.isTerminated == "1"){
                            ...
                            switch(Number(data.tenderStatus)){
                                case bidModel.CONSTANTS.tenderStatus.TENDER.key:
                                    return bidModel.CONSTANTS.tenderStatus.TENDER.value;
                                    break;
                                case bidModel.CONSTANTS.tenderStatus.BIDOPEN.key:
                                    return bidModel.CONSTANTS.tenderStatus.BIDOPEN.value;
                                    break;
                                case bidModel.CONSTANTS.tenderStatus.END.key:
                                    return bidModel.CONSTANTS.tenderStatus.END.value;
                                    break;
                                case bidModel.CONSTANTS.tenderStatus.SIGN.key:
                                    return bidModel.CONSTANTS.tenderStatus.SIGN.value;
                                    break;
                                case bidModel.CONSTANTS.tenderStatus.INSTANCE.key:
                                    return bidModel.CONSTANTS.tenderStatus.INSTANCE.value;
                                    break;
                                case bidModel.CONSTANTS.tenderStatus.WASTE.key:
                                    return bidModel.CONSTANTS.tenderStatus.WASTE.value;
                                    break;
                                case bidModel.CONSTANTS.tenderStatus.FLOWMARK.key:
                                    return bidModel.CONSTANTS.tenderStatus.FLOWMARK.value;
                                    break;
                            }
                        }
                    }
            ...
            },
            {
                ...
                itemRenderer:{
                    render: function(data, field, index, grid){
                        return (data.tenderType == bidModel.CONSTANTS.receiver.OPEN.key) ? bidModel.CONSTANTS.receiver.OPEN.value:bidModel.CONSTANTS.receiver.INVITATION.value;
                    }
                }
            },
            {
                    ...
                    render: function (data, field, index, grid) {
                        return $("<a href='' class='orange-font'>详情</a>")
                            .on("click", function (event) {
                                ...
                                X.publish(X.CONSTANTS.channel.menuCall,{m:"bid.tenderDetail",para:{tenderId:data.tenderId}});
                            })[0];
                    }
                },
                width:"4%",
    ...
    var schemas = (function(){
        var schemas = {
            "tabAll" :{
                    ...
                    schema: {
                        simple:[
                            ...
                            },
                            {
                                ...
                                title:"招标状态",
                                ctrlType:"ComboBox",
                                dataSource: bidModel.CONSTANTS.status,
                                className : "mr66 w160 m0 va_8",
                                defaultValue:""
                            }
                        ]
                    },
                ...
                url : X.config.bid.api.myBidListByPage
            }
        };
        return schemas;
    })();
    ...
    var route = new routerHelper("bid.myBid",schemas,getRoute);
    return ctrl;
});
