...
<template>
    <div class="plan-list-container billing-list">
        <template-search-list ref="pageTmpl" title="费用项" :simple="simpleSearch" :combo="comboSearch"
                              :method="queryList"
                              :filter="quickFilter">
            <template slot="page-table-operations">
                <nt-popover style="background: #fff;" ref="batchdelete" placement="bottom"
                    ...
                    <div class="bgcfff">
                        <div>
                            <span v-if="ids.length>0">是否删除此费用项？</span>
                            <span v-else>请选择要删除的费用项</span>
                        </div>
                ...
                <nt-button type="primary" nt-col-offset-0 @click="jumpAdd">新增费用项</nt-button>
            ...
            <template slot="page-table" slot-scope="props">
                <div class="contentBox">
                    <nt-table :data="props.tableData" @select="selectIds" @select-all="selectIds" class="border_layout">
                        ...
                        <nt-table-column prop="expenseName" label="费用项名称" show-overflow-tooltip>
                            <template slot-scope="scope">
                                {{cutMoreChat(scope.row.expenseName, 20)}}
                            </template>
                        </nt-table-column>
                        <nt-table-column prop="receiptType" label="关联单据类型" show-overflow-tooltip>
                            <template slot-scope="scope">
                                <span v-if="scope.row.receiptType === '01'">报关单</span>
                                <span v-if="scope.row.receiptType === '02'">清关单</span>
                            </template>
                        </nt-table-column>
                        <nt-table-column prop="expenseDesc" label="费用项描述" show-overflow-tooltip>
                            <template slot-scope="scope">
                                {{cutMoreChat(scope.row.expenseDesc, 40)}}
                            </template>
                        </nt-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            queryList: async function (params) {
                return await this.ctx.models.costitem.getListByPage(params);
            },

            //删除单条开票厂商
            handleDeleteCurrent: async function (row) {
                await this.ctx.models.costitem.deleteCurrent({id: row.id});
            ...
            handlerBatchDelete: async function () {
                if (this.ids.length > 0) {
                    const res = await this.ctx.models.costitem.batchDelete({ids: this.ids});
