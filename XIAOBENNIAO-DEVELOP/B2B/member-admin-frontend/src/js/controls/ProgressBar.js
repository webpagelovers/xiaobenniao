(function ($, X) {
    ...
    X.prototype.controls.widget("ProgressBar", function (controlType) {
        ...
        ProgressBar.prototype.createTemplate = function () {
            var progressBar = '<ul class="mt70 ml30">';
        ...
        ProgressBar.prototype.getNowStep = function () {
            ...
            for (var i = 0; i < this.nowStep; i++) {
                if (i == this.nowStep - 1) {
                    var html = '<div class="indexCircleWrap mt_9"><div class="indexCircle bgcDarkBlue">' + this.progressContent[i] + '</div></div>' +
                        '<p class="poa t50 w90 l6">' + this.progressContentInfo[i] + '</p>';
                    aLi.eq(i).html(html);
                } else {
                    var html = '<div class="indexCircle mr10 ml10 bgcDarkBlue">' + this.progressContent[i] + '</div>' +
                        '<p class="poa t50 w80 col66 l6">' + this.progressContentInfo[i] + '</p>';
                    aLi.eq(i).html(html);
