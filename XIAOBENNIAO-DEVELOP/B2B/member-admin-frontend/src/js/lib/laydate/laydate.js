...
//全局配置，如果采用默认均不需要改动
var config =  {
    path : X.config.PATH_FILE.path.root + "/js/lib/laydate/",
    fixed : true,
...
Dates.orien = function(obj, pos){
    ...
    // 判断位置是否超过window的宽度
    if((rect.left+obj.offsetWidth)>document.body.offsetWidth){
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        +'</div>';
        //Dates.elem.parentNode.appendChild(div);
        //Lorry 修改，将该DIV放置于body中，避免在单页面应用中，被移除
