X.define("modules.common.ueditorImageUploader",["adapter.webuploader","adapter.ueditor"],function () {
	...
	var imageUploader = function(Editor){
		var opt = {
			...
			type : imageUploader.type || 17,  //上传图片传给后端类型
			size : imageUploader.size || 5,   //上传图片大小
			width : imageUploader.width || 1000,  //上传成功后在编辑器展示的宽度
			height : imageUploader.height || 1000, //上传成功后在编辑器展示的高度
		...
		uploader.on( 'uploadSuccess', function( file,response) {
			if(response.data){
				...
				var width = (opt.width = 1000) ? (file._info ? (file._info.width > opt.width ? opt.width : file._info.width)  : '100') : opt.width;
				var htight = (opt.height = 1000) ? (file._info ? (file._info.height > opt.width ? opt.width : file._info.height) : '100') : opt.height;
				//图片上传成功后，在编辑器光标处插入
