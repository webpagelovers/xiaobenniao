;(function (_$) {
    ...
    //验证合同名称是否被注册
    _$.validator.addMethod("isProductName", function (value, element) {

        var flag = 1;

        _$.ajax({
            type: "POST",
            url: userUrl + "/product/verifyProduct",
            data: toStringify({
                'nameCn': value
            }),
            async: false,
    ...
    //验证合同名称是否被注册
    _$.validator.addMethod("isContractName", function (value, element) {

        var flag = 1;

        _$.ajax({
            type: "POST",
            url: userUrl + "/contract/"+value,
            async: false,
            success: function (data) {
                if (data.statusCode == '2000000') {
                    flag = 0;
    ...
    _$.validator.addMethod("isLicenseNumber", function (value, element) {
        ...
        _$.ajax({
            ...
            success: function (data) {
                ...
                }else if(data.statusCode == '1000100'){
                    window.location.href =window.X.config.receptionUrl.loginUrl
                }
            }
        });
    ...
    _$.validator.addMethod("isPassWord", function (value, element) {
        ...
        _$.ajax({
            type: "POST",
            url: userUrl + "api/user/pwd/verify",
            dataType: "json",
            async: false,
            data: toStringify({
    ...
    //大写英文或数字
    _$.validator.addMethod("bigCharOrNum", function (value, element) {
    ...
    //验证用户是否已绑定企业
    _$.validator.addMethod("checkUserIsBindCompany", function (value, element) {
    ...
    //验证公司名称是否已经注册
    _$.validator.addMethod("isCompanyName", function (value, element) {

        var flag = 1;

        _$.ajax({
            type: "POST",
            url: userUrl + "/company/check",
            async: false,
            data: toStringify({
                'companyNameCn': value
            }),
    ...
    // 最大价格9999999999.99
    _$.validator.addMethod("maxPrice", function (value, element) {
        return this.optional(element) || (Number(value) < Number('9999999999.99'));
    }, "最大价格9999999999.99");
    ...
    // 匹配english和空格
    _$.validator.addMethod("isEnglishBackSpace", function (value, element) {
        return this.optional(element) || /^[A-Za-z\s]*$/.test(value);
    }, "匹配english");
    ...
    // 不包含中文字符
    _$.validator.addMethod("isNoChinese", function (value, element) {
        return this.optional(element) || /^[^\u4e00-\u9fa5]+$/.test(value);
    }, "不能包含中文字符。");
    ...
    $.validator.addMethod("WmsSku", function (value, element,pramas) {
        ...
        _$.ajax({
            type : "get",
            url : userUrl + "owms/commodity/infoBySku?sku="+value,
            async : false, //同步方法，如果用异步的话，flag永远为1
            success : function (data) {
                if (data.statusCode == '2000000') {
                    if(!data.data){
    ...
    _$.validator.addMethod("tempName", function (value, element) {
        ...
        _$.ajax({
            ...
            url: userUrl + "js/custom/data/template/addCreate.json",
            dataType: "json",
            async: false,
            success: function (data) {
    ...
    //多个输入框值不能相同
    jQuery.validator.addMethod("exclusion", function (value, element) {
        var values = []
        for (var i in this.settings.rules) {
            var item = this.settings.rules[i],
                val  = $('[name='+ i +']').val()

            item.exclusion && val && element.name !== i && values.push(val)
        }
