    ...
    X.prototype.controls.widget("Searcher",function (controlType) {
        ...
        */
        Searcher.prototype.fireSearch = function (click) {           
            ...
            if(JSON.stringify(data)!=JSON.stringify(this._oldData)){
                ...
                if(this.onSearch){
                    this.options.schema.simple.forEach(function (item) {
                        item.placeholder === data[item.name] && delete data[item.name]
                    })
        ...
        */
        Searcher.prototype.getFieldHtml = function(item) {
            ...
            var type = item["association"]? '<span class="search-association">——</span>': item["title"]? '：': ' '
            var html='<dl class="screen_box fL"><dt>' + (item["title"] || '') + type +'</dt>';
