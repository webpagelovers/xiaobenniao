(function (factory) {	
	...
	$.fn.mvalidate=function(options){
		...
		return this.mvalidateDestroy().each(function(event) {
            ...
            if(flag && opts.onFocusout){
                $fields.filter(type[0]).each(function() {

                    $(this).on("focusout."+namespace,function(event){
                        validateField.call(this,event,opts)
                    })
                });
            }
	...
	$.fn.mvalidateDestroy=function(){
		...
		if($form.is('form') && $.isPlainObject(dataValidate) && dataValidate.options && typeof(dataValidate.options.namespace) == 'string') {
