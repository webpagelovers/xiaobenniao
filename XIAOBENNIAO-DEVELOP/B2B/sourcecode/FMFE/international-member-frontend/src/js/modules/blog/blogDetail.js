X.define('modules.blog.blogDetail', ['modules.common.global','model.blogModel', 'common.share', 'modules.user.login', 'modules.user.regist'], function (global, model, share, loginView, registView) {
    ...
    var events = {
        init: function() {
            ...
            //查看是否有comment数据
            var params = X.parseURL(location.href).params
            params.comment && (commentTextarea.val(params.comment), window.scrollTo(0, document.body.scrollHeight))
        },
            ...
            if (commentForm.valid()) {
                ...
                model.postComments(data, function(result) {
                    if (result.statusCode === X.CONSTANT.statusCode.SUCCESS){
                        ...
                        //loginView.showView()
                        location.href = 'user/login.html?go=' + location.href + '&comment=' + commentTextarea.val()
                    }
        ...
        setValidate: function() {
            validate = {
                ...
                messages: {
                    commentTextarea:    { required: $.i18n.prop('blog_comment_val_require'), rangelength: $.i18n.prop('blog_comment_val_max'), ridCh: $.i18n.prop('blog_comment_val_english')}
                },
                onkeyup: false,
