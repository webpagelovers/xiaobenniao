...
(function($, undefined) {
		...
		return {
			...
			isOverflow: function(ele) {
				...
				var attrMin = $(ele).attr("min"), attrMax = $(ele).attr("max"), attrStep
					...
					,attrCnDataMin,attrCnDataMax
					// 值
					, value = ele.value;

				if (!attrMin && !attrMax) {
					attrDataMin = $(ele).attr("data-min"), attrDataMax = $(ele).attr("data-max"),
					attrCnDataMin=$(ele).attr("data-cnmin"),attrCnDataMax=$(ele).attr("data-cnmax");
					if(attrCnDataMin || attrCnDataMax){
						value=value.replace(/[\u4e00-\u9fa5]/g,'**');
						if(attrCnDataMin){
							attrDataMin=attrCnDataMin;
						}
						if(attrCnDataMax){
							attrDataMax=attrCnDataMax;
						}
					}
	...
	$.fn.extend({
		...
		testRemind: function(content, options) {
			...
			//yangyang add patternText
			var title = $(this).attr("patternText");
			if(title){
				title = "<br />" + title;
			} else{
				title = "";
			...
			this.remind = $('<div id="'+ $.testRemind.id +'">'+ content + title +'</div>').css(params.css);
