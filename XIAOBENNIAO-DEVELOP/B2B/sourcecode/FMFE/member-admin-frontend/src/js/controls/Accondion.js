...
X.prototype.controls.widget("Accondion",function (controlType) {
    ...
    Accondion.prototype.expandRecursive = function (data,click) {
    	var li,pnode,pli;
        if(data){                        
            li = this.getItem(data);
            if(li && li.length>0){
                ...
                if(pnode){
                    do{
                        pli = this.getItem(pnode);
                        if(pli && pli.length>0){
                            this.expand(pnode,true);
                            pnode = pnode.parent;
                        }                        
                    }while(pnode && pli && pli.length>0 && pli.data("level")>0)    
                }                

                this.expand(data,click);
            }            
        }
    };


    Accondion.prototype.expand = function (data,click) {
    ...
    Accondion.prototype.setSelectedItem = function (data) {
    	...
    	if(li && li.length>0){
            ...
            this.expandRecursive(data,false);
    	}
    };
    ...
    Accondion.prototype.getItem = function (data) {
        var item = this.$elem.find("li[code='" + this.getMap(data,"code") + "']");
    ...
    */
    Accondion.prototype.getFromResults = function (results, level) {
        ...
        for (i = 0; i < numResults; i++) {
            ...
            if (first === false) {
                first = String(this.getMap(result,"code"));
                $first = $li;
    ...
    */
    ...
    Accondion.prototype.getLiItemFromResultWithChild = function (result, level) {
        ...
        var $div = $('<div class="header"><span class="arrow"><i class="iconfont icon-right"></i></span><span class="txt"><i class="iconfont '+this.getMap(result,"ico")+'"></i><a data-x-href="'+ this.getMap(result,"code") +'">' + this.getMap(result,"name") + '</a></span></div>');
        ...
        if ($.isArray(result.children) && result.children.length>0) {            
            for (var i = 0; i < result.children.length; i++) {
                ...
                if(child.visible==undefined || child.visible==true || child.visible=="true"){
        ...
        var $li = $("<li leaf='0' code=" + this.getMap(result,"code") + "></li>");
        $li.append($div);
        $li.append($ul);
        $li.data("data", result);
        return $li;
    ...
    Accondion.prototype.getMoreLi = function (result, level) {
        var $li = $('<li class="more" pcode="' + this.getMap(result,"code") + '"><span>更多</span></li>');
        $li.data("data", result);
        $li.data("level", level);
        var self = this;
        return $li;
    ...
    Accondion.prototype.getMoreItem = function (data) {
        var item = this.$elem.find("li[pcode='" + this.getMap(data,"code") + "']");
    ...
    */
    ...
    Accondion.prototype.afterItemClick = function (element) {
        if (element.children("ul").css("display") == "block") {
            ...
            element.children("div").find(".arrow i").removeClass("icon-down");
        }
        else {
            element.children("ul").slideDown(300);
            element.children("div").addClass("menu-show");
            element.children("div").find(".arrow i").addClass("icon-down");
        }
        if (this.$scroller)
            this.$scroller.refresh();
    ...
    */
     ...
     * getMappingCode
     */
    Accondion.prototype.getMap = function (data,attr) {
        try{
            var result = data[this.options.map[attr]];
            if(result){
                return result;
            }
            return data[attr] || "";    
