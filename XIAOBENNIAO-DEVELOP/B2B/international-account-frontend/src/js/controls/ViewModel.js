(function ($,X) { 
    ...
    ViewModel.prototype.fillData = function() {
        ...
        if (this.id && this.query) {
            this.query(this.id, function(res) {
                var data = me._redefineData(res.data[0]),
            ...
            this.submit && this.submit(data, function(res) {
                if (res.statusCode === window.X.CONSTANTS.statusCode.SUCCESS) {
                    layer.open({
                        closeBtn: 0,
                        title: '提示',
                        content: '<span style="margin-left:20px;">提交成功</span>',
                        yes: function(index){
                            layer.close(index)
                            window.X.router.back()
                        }
                    })
    ...
    */
    ...
    /**
     @method _integrated 用户自重新整合后端返回的数据
    */
    ViewModel.prototype._redefineData = function(data) {
        var redefine
        (redefine = this.redefineData) && (data = redefine(data))
    ...
    /**
     @method _ready 完成事件
    */
    ViewModel.prototype._ready = function() {
        var ready
        (ready = this.ready) && ready.call(this)
    }
